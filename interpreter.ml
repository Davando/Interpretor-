...
and ast_ize_stmt (s:parse_tree) : ast_s =
  ...
  | PT_nt ("S", [PT_term "if"; rel; sl; PT_term "fi"]) -> AST_if ((ast_ize_expr rel), (ast_ize_stmt_list sl))
  | PT_nt ("S", [PT_term "do"; sl; PT_term "od"]) -> AST_do (ast_ize_stmt_list sl)
...
and ast_ize_expr (e:parse_tree) : ast_e =
  ...
  | PT_nt ("T", [fact; fact_tail]) -> ast_ize_expr_tail (ast_ize_expr fact) (fact_tail)
...
and ast_ize_expr_tail (lhs:ast_e) (tail:parse_tree) : ast_e =
  ...
  | PT_nt ("FT", []) -> lhs
  | PT_nt ("FT", [mo; fact; fact_tail]) -> AST_binop (ast_ize_op mo, lhs, ast_ize_expr_tail (ast_ize_expr fact) (fact_tail))
